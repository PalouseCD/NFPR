...
params:
  WATERSHED_ID: 
    value: "PR" #put watershed abrev here! (CT for Cow & Thorn, KT for Kamiache & Thorn, PR for Palouse River, or ST for Steptoe Canyon)
    
### Make sure to change the name in output_file to desired report name ###
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_dir = "R:/_04_Project_Data/R/NFPR/RMD" , output_file = paste("PR_Report-", Sys.Date(), ".docx", sep=''))}) #Output name must be manually named
...
if(WATERSHED_ID == "PR"){
  WATERSHED_NAME = "Palouse River"
}

if(WATERSHED_ID == "ST"){
  WATERSHED_NAME = "Steptoe Canyon"
}

if(WATERSHED_ID == "SFC"){
  WATERSHED_NAME = "Spring Flat Creek"
}
...
if (WATERSHED_NAME == "Palouse River"){
  MASTER = read.csv("R:/_04_Project_Data/R/NFPR/outputs/PRTribs_Daily_Averages_2001-06-05_to_2023-03-06_.csv")
}

if (WATERSHED_NAME == "Cow and Thorn Creeks"){
  MASTER = read.csv("R:/_04_Project_Data/from_WISKI/WQ_Projects/CowThorn/CowThorn_2018-11-15_to_2022-09-29_.csv")
}
  
if (WATERSHED_NAME == "Steptoe Canyon"){
  MASTER = read.csv("R:/_04_Project_Data/from_WISKI/WQ_Projects/Steptoe/Steptoe_2020-07-28_to_2022-07-11_.csv") 
}  

if (WATERSHED_NAME == "Spring Flat Creek"){
  MASTER = read.csv("R:/_04_Project_Data/from_WISKI/NFPR/NFPR_Combined/SFC0.35/SFC0.35_2020-02-19_to_2022-07-25_.csv")
}
...
The figure below displays air temperature ($^\circ$C) from `r FIRST_SAMPLE` to `r LAST_SAMPLE`. Temperature is measured monthly in the field using a thermometer or a Hobo air temperature sensor.
...
### **Turbidity** 
The figure below displays a measure of water clarity, measured in nephelometric turbidity units (NTU), from `r FIRST_SAMPLE` to `r LAST_SAMPLE`. Turbidity is measured by determining how much light can pass through the water. Water clarity directly affects ecological productivity and habitat quality in aquatic environments. Excessive turbidity can be caused by physical disruption of the stream channel or through sediment inputs as a result of runoff and upland erosion.  Turbidity is measured every 15 minutes using a handheld YSI DSSPro.

```{r Turbid_graph, echo=FALSE, message=FALSE, warning=FALSE, fig.width=14, fig.height=12}

TEMP = MASTER[,c(1,2, 18)]
...
### **Ammonia** 
The figure below displays ammonia concentration, measured in milligrams per liter (mg/L), from `r FIRST_SAMPLE` to `r LAST_SAMPLE`. Ammonia is a common form of nitrogen found in streams, mostly originating from sewage systems and fertilizer applications It is harmful to fish and other aquatic organisms with a concentration of less than 1 mg/L. Ammonia concentrations are derived from monthly in-stream water samples.   
...
### **Orthophosphate**
The figure below displays orthophosphate (OP) concentrations, in milligrams per liter (mg/L), from `r FIRST_SAMPLE` to `r LAST_SAMPLE`. In addition to the total phosphorus values displayed above, orthophosphates, the dissolved form phosphorous, can provide an estimate of how much phosphorus is avilable to aquatic plants. OP can also aid in the identification of pollution sources since it is commonly derived from fertilizers or other anthropogenic sources. Orthophosphate samples are collected monthly in-stream.
